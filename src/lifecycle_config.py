...
curl -LO --output-dir /var/tmp/ \
    https://github.com/aws-samples/sagemaker-studio-apps-lifecycle-config-examples/releases/download/v$ASI_VERSION/$PYTHON_PACKAGE
...
sudo /bin/bash -c "echo 'AWS_CONTAINER_CREDENTIALS_RELATIVE_URI=$AWS_CONTAINER_CREDENTIALS_RELATIVE_URI' \
    >> /etc/environment"

# Add script to crontab for root.
echo "Adding autostop idle Python script to crontab..."
echo "*/2 * * * * /bin/bash -ic '$CONDA_HOME/python $PYTHON_SCRIPT_PATH --time __AUTO_STOP_IDLE_TIME__ \
    --region $AWS_DEFAULT_REGION >> $LOG_FILE'" | sudo crontab -
"""
...
if [ ! -f ~/.initial_setup ]; then
    ...
    echo "deb [signed-by=/usr/share/keyrings/corretto-keyring.gpg] \
        https://apt.corretto.aws stable main" | sudo tee /etc/apt/sources.list.d/corretto.list
    sudo apt update -qq
